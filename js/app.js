...
BranchInfo.prototype.salesPerHour = function () {
    for (var i = 0; i < storeHours.length; i++) {
        var x = Math.floor(Math.random() * (this.maxCus - this.minCust + 1) + this.minCust);
        var y = x * this.avSale;
        this.storeCokie.push(Math.floor(y));
        this.onDayCookie += Math.floor(y);
    }
    this.storeCokie.push(this.onDayCookie);
};
...
BranchInfo.prototype.tableBody = function () {
    ...
    for (var i = 0; i < store.length; i++) {
        ...
        for (var i = 0; i < this.storeCokie.length; i++) {
            var tdEl = document.createElement('td');
            dataRowEl.appendChild(tdEl);
            tdEl.textContent = this.storeCokie[i];
        }
    }
};
